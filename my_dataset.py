...
class DriveDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        manual = np.array(manual) / 255   # 0/255 ->0/1
        roi_mask = Image.open(self.roi_mask[idx]).convert('L')
        roi_mask = 255 - np.array(roi_mask)
        mask = np.clip(manual + roi_mask, a_min=0, a_max=255)  # foreground（manual） 1, background 0，忽略 255
        ...
        return img, mask   # img 为RGB图像，mask为灰度图像
